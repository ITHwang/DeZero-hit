...
class Transpose(Function):
    ...
    def backward(self, gy):
        ...
        inv_axes = tuple(
            np.argsort([ax % axes_len for ax in self.axes])
        )  # same as self.axes, ax % axes_len: check indices.
...
class SumTo(Function):
    ...
    def forward(self, x):
        ...
        y = utils.sum_to(
            x, self.shape
        )  # Don't be confused utils.sum_to and functions.sum_to
        return y
...
# for simple computational graph (memory optimization)
